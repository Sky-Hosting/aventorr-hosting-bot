...
module.exports = async (client, message, args) => {
    if (!message.member.roles.cache.has(config.roleID.donor)) return message.channel.send('You need to be donor to run the cmd');
     ...
     if(!args[1] || args[1]?.toLowerCase() === 'list'){  
            ...
            const noTypeListed = new Discord.MessageEmbed() 
                ...
                .addField(`**Game Servers__**:   `, `>>> bedrock\n forge \n paper \n pocketminemp \n spigot`, true)
        ...
        try{
            ServerData = require(`../../server_creation_donor/${args[1]?.toLowerCase()}.js`)(userData.get(message.author.id).consoleID, srvname ? srvname : args[1], config.settings.serverCreation_donor)
        }catch(err){
